...
export class MaintenanceScene extends Phaser.Scene{
  ...
  create():void{
    ...
    this.retryText=this.add.text(270,672,"REINTENTAR",{fontFamily:"system-ui, sans-serif",fontSize:"17px",fontStyle:"bold",color:"#f5f7fa"}).setOrigin(.5);
    ...
    this.add.text(270,748,"La pantalla se actualizará sola cuando vuelva el servicio.",{fontFamily:"system-ui, sans-serif",fontSize:"11px",color:"#71818d",align:"center",wordWrap:{width:390}}).setOrigin(.5);
    this.renderStatus();
  ...
  private async check():Promise<void>{
    ...
    if(!next.maintenance){this.scene.start("menu");return;}
    this.renderStatus();this.retryText.setText("REINTENTAR");this.retryBg.setInteractive({useHandCursor:true});this.checking=false;
  }
}
