...
export class CommunityPublishScene extends Phaser.Scene{
  ...
  create():void{
    ...
    const drafts=CommunityDrafts.list();
    if(!drafts.length){
      this.add.text(270,350,"NO TIENES BORRADORES GUARDADOS",{fontFamily:"system-ui",fontSize:"15px",fontStyle:"bold",color:"#cbd7df"}).setOrigin(.5);
      this.add.text(270,390,"Crea un mapa en BETA LAB y pulsa GUARDAR.\nEl autosave del editor no se publica por accidente.",{fontFamily:"system-ui",fontSize:"11px",color:"#8295a2",align:"center",lineSpacing:5}).setOrigin(.5);
      this.button(270,478,310,"CREAR MAPA",()=>this.scene.start("editor"),true);sharpenSceneText(this);return;
    }
    ...
    this.add.text(270,918,"Un mapa debe completarse en playtest antes de publicarse",{fontFamily:"system-ui",fontSize:"9px",color:"#6f818d"}).setOrigin(.5);
  ...
  private draftCard(draft:SavedCommunityDraft,y:number):void{
    ...
    const select=():void=>{this.selectedId=draft.id;for(const child of this.children.list)if(child.getData("draft-card"))((child as Phaser.GameObjects.Rectangle)).setStrokeStyle(2,child===bg?0x79a4bd:0x30414e);bg.setData("draft-card",true);this.showActions(draft);};
  ...
  private showActions(draft:SavedCommunityDraft):void{
    ...
    const tested=draft.playtestedAt!==null;
    ...
    children.push(this.actionButton(270,866,180,"BORRAR BORRADOR",()=>{if(window.confirm(`¿Borrar ${draft.name}?`)){CommunityDrafts.remove(draft.id);this.scene.restart();}},false));
