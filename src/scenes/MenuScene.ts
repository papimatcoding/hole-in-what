...
export class MenuScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.makeModeButton("CLASSIC", "classic", 300);
    this.makeModeButton("TROLL", "troll", 415);
    this.makeSimpleButton("PERSONALIZAR", 545, () => {
      this.scene.start("cosmetics");
    });
    this.makeSimpleButton("RECOMPENSAS", 630, () => {
      this.scene.start("rewards");
    });

    const equipped = SaveSystem.cosmetics().equipped;
    this.add.text(DESIGN_WIDTH / 2, 715, `● ${equipped.ball.replace("ball-", "")}   ·   ─ ${equipped.trail.replace("trail-", "")}`, {
      fontFamily: "system-ui, sans-serif",
      fontSize: "11px",
      color: "#667687"
    }).setOrigin(0.5);

    this.add.text(DESIGN_WIDTH / 2, DESIGN_HEIGHT - 70, "V6 · progresión", {
      fontFamily: "system-ui, sans-serif",
      fontSize: "12px",
      color: "#657282"
    }).setOrigin(0.5);
  ...
  private makeModeButton(label: string, mode: GameMode, y: number): void {
    ...
    if (locked) {
      ...
      this.add.text(105, y + 22, `Consigue ${progress.requiredStars}★ en Classic o completa ${progress.total}/${progress.total}`, {
        fontFamily: "system-ui, sans-serif",
        fontSize: "10px",
        color: "#657282"
      }).setOrigin(0, 0.5);
  ...
  private makeSimpleButton(label: string, y: number, action: () => void): void {
    const bg = this.add.rectangle(270, y, 390, 68, 0x151d25)
