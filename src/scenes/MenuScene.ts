...
import { DESIGN_HEIGHT, DESIGN_WIDTH, setupDesignCamera, sharpenSceneText, uiFontSize } from "../config/display";
...
export class MenuScene extends Phaser.Scene {
  ...
  create():void{
    ...
    this.add.text(DESIGN_WIDTH/2,150,"MINIGOLF · 3 ESTRELLAS",{fontFamily:"system-ui, sans-serif",fontSize:uiFontSize(12,2),color:"#8f9dab"}).setOrigin(.5);

    const alias=BetaTelemetry.alias();
    const identity=this.add.text(DESIGN_WIDTH/2,178,alias?`JUGADOR · ${alias}   ✎`:"JUGADOR · ELIGE TU NOMBRE   ✎",{fontFamily:"system-ui, sans-serif",fontSize:uiFontSize(11,2),fontStyle:"bold",color:alias?"#c9d9e3":"#e0bc70"}).setOrigin(.5).setInteractive({useHandCursor:true});
    identity.on("pointerup",()=>this.scene.start("player-profile"));
    if(BETA_TESTING)this.add.text(DESIGN_WIDTH/2,202,"BETA TEST · TODOS LOS HOYOS ABIERTOS",{fontFamily:"system-ui, sans-serif",fontSize:uiFontSize(10,2),fontStyle:"bold",color:"#8fb8cf"}).setOrigin(.5);
    ...
    this.add.text(DESIGN_WIDTH/2,794,`● ${equipped.ball.replace("ball-","")}   ·   ─ ${equipped.trail.replace("trail-","")}`,{fontFamily:"system-ui, sans-serif",fontSize:uiFontSize(10,2),color:"#718090"}).setOrigin(.5);

    const beta=this.add.rectangle(270,842,390,48,0x111922).setStrokeStyle(2,0x405666);
    const betaText=this.add.text(270,842,`BETA LAB · EDITOR   ·   ${BetaFeedbackSystem.count()} FB`,{fontFamily:"system-ui, sans-serif",fontSize:uiFontSize(11,2),fontStyle:"bold",color:"#b9c9d4"}).setOrigin(.5);
    this.wirePress(beta,betaText,842,410,58,()=>this.scene.start("editor"),0x111922,0x1d2b36);

    this.add.text(DESIGN_WIDTH/2,DESIGN_HEIGHT-28,BETA_TESTING?"CAMPAÑA BETA · TESTER MODE":"CAMPAÑA · CORE SLICE",{fontFamily:"system-ui, sans-serif",fontSize:uiFontSize(10,2),color:"#657282"}).setOrigin(.5);
  ...
  private makeModeButton(label:string,mode:GameMode,y:number):void{
    ...
    const progress=this.add.text(435,y+15,BETA_TESTING?`BETA · ★ ${stars} / ${levels.length*3}`:`★ ${stars} / ${levels.length*3}`,{fontFamily:"system-ui, sans-serif",fontSize:uiFontSize(13,1),color:"#c9d4df"}).setOrigin(1,.5);
