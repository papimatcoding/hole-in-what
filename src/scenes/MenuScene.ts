...
export class MenuScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(DESIGN_WIDTH/2,DESIGN_HEIGHT-48,"V8.3 · procedural courses",{
  ...
  private makeModeButton(label:string,mode:GameMode,y:number):void {
    const levels=v82LevelsForMode(mode);
    ...
    if(locked){
      ...
      this.add.text(435,y-12,"BLOQUEADO",{fontFamily:"system-ui, sans-serif",fontSize:"10px",fontStyle:"bold",color:"#7a8793"}).setOrigin(1,0.5);
      this.add.text(105,y+22,`Consigue ${p.requiredStars}★ o completa Classic`,{fontFamily:"system-ui, sans-serif",fontSize:"10px",color:"#657282"}).setOrigin(0,0.5);
      this.add.text(435,y+22,`${p.stars}/${p.requiredStars}★`,{fontFamily:"system-ui, sans-serif",fontSize:"12px",fontStyle:"bold",color:"#8d98a3"}).setOrigin(1,0.5);
      return;
    }
  ...
  private makeSimpleButton(label:string,y:number,action:()=>void):void {
    ...
    const text=this.add.text(270,y,label,{fontFamily:"system-ui, sans-serif",fontSize:"14px",fontStyle:"bold",color:"#d7e0e8"}).setOrigin(0.5).setInteractive({useHandCursor:true});
