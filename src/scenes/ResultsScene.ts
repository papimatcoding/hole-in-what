...
export class ResultsScene extends Phaser.Scene {
  ...
  create():void{
    ...
    this.add.text(498,54,`◈ ${reward.totalCoins}   ◆ ${reward.totalGems}`,{fontFamily:"system-ui, sans-serif",fontSize:"14px",fontStyle:"bold",color:"#d9e4ee"}).setOrigin(1,.5);
    ...
    if(notices.length>0){
      const notice=this.add.text(270,347,notices.join("  ·  "),{fontFamily:"system-ui, sans-serif",fontSize:"11px",fontStyle:"bold",color:"#dfe9ef",align:"center",wordWrap:{width:450}}).setOrigin(.5).setAlpha(0);
    ...
    this.add.text(92,389,"MAESTRÍA",{fontFamily:"system-ui, sans-serif",fontSize:"11px",fontStyle:"bold",color:"#71808e"}).setOrigin(0,.5);
    this.add.text(108,421,`${metThree?"✓":"·"}  ★★★   ${formatRequirement(level.threeStar)}`,{fontFamily:"system-ui, sans-serif",fontSize:"13px",fontStyle:"bold",color:metThree?"#f1d07a":"#778591"}).setOrigin(0,.5);
    this.add.text(108,455,`${metTwo?"✓":"·"}  ★★     ${formatRequirement(level.twoStar)}`,{fontFamily:"system-ui, sans-serif",fontSize:"12px",color:metTwo?"#c5d0da":"#6f7b86"}).setOrigin(0,.5);
    ...
    if(target!==null&&target!==undefined){
      ...
      this.add.text(270,510,`Te ${gap===1?"faltó 1 golpe":`faltaron ${gap} golpes`} para ${goalStars}`,{fontFamily:"system-ui, sans-serif",fontSize:"14px",fontStyle:"bold",color:"#c9d3db"}).setOrigin(.5);
    }else{
      this.add.text(270,510,"Ruta de maestría conseguida",{fontFamily:"system-ui, sans-serif",fontSize:"14px",fontStyle:"bold",color:"#f1d07a"}).setOrigin(.5);
    }
    ...
    if(reward.newlyUnlockedCosmetics.length>0){const names=reward.newlyUnlockedCosmetics.map(id=>cosmeticById(id)?.name).filter((name):name is string=>Boolean(name));this.add.text(270,infoY,`DESBLOQUEADO · ${names.join(" · ")}`,{fontFamily:"system-ui, sans-serif",fontSize:"11px",fontStyle:"bold",align:"center",color:"#f1d07a",wordWrap:{width:430}}).setOrigin(.5);}
    ...
    this.add.text(270,815,"NIVELES",{fontFamily:"system-ui, sans-serif",fontSize:"15px",fontStyle:"bold",color:"#aeb9c5"}).setOrigin(.5).setInteractive({useHandCursor:true}).on("pointerup",()=>this.scene.start("level-select",{mode:this.resultData.mode,page:Math.floor(this.resultData.levelIndex/10)}));
  ...
  private makeButton(label:string,y:number,action:()=>void,primary:boolean):void{
    ...
    const text=this.add.text(270,y,label,{fontFamily:"system-ui, sans-serif",fontSize:"17px",fontStyle:"bold",color:"#f5f7fa"}).setOrigin(.5).setInteractive({useHandCursor:true});
