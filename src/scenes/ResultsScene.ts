...
export class ResultsScene extends Phaser.Scene{
  ...
  private openFeedback():void{
    ...
    choices.forEach(([label,category],i)=>{const x=170+(i%2)*200,y=386+Math.floor(i/2)*68,bg=this.add.rectangle(x,y,178,54,0x1a2731).setStrokeStyle(1,0x496173).setInteractive({useHandCursor:true});const t=this.add.text(x,y,label,{fontFamily:"system-ui",fontSize:"11px",fontStyle:"bold",color:"#dfe9ef"}).setOrigin(.5);children.push(bg,t);bg.on("pointerdown",()=>{bg.setScale(.98);t.setScale(.98);}).on("pointerout",()=>{bg.setScale(1);t.setScale(1);}).on("pointerup",()=>{bg.setScale(1);t.setScale(1);void this.saveFeedback(category);});});
  ...
  private feedbackCloseButton(children:Phaser.GameObjects.GameObject[],x:number,y:number,label:string,action:()=>void):void{const bg=this.add.rectangle(x,y,180,48,0x141d24).setStrokeStyle(1,0x354652).setInteractive({useHandCursor:true}),t=this.add.text(x,y,label,{fontFamily:"system-ui",fontSize:"11px",fontStyle:"bold",color:"#94a5b1"}).setOrigin(.5);children.push(bg,t);bg.on("pointerdown",()=>{bg.setScale(.98);t.setScale(.98);}).on("pointerout",()=>{bg.setScale(1);t.setScale(1);}).on("pointerup",()=>{bg.setScale(1);t.setScale(1);action();});}
