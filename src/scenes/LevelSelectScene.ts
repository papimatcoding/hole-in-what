...
export class LevelSelectScene extends Phaser.Scene {
  ...
  create():void{
    ...
    this.add.text(270,88,`GRUPO ${group}  ·  ${pageStart+1}–${Math.min(pageStart+10,levels.length)}`,{fontFamily:"system-ui, sans-serif",fontSize:"14px",fontStyle:"bold",color:this.mode==="troll"?"#d5a66d":"#7fa8c2"}).setOrigin(.5);
    const totalStars=SaveSystem.totalStars(levels.map(level=>level.id)),unlocked=SaveSystem.unlockedLevelCount(this.mode);
    this.add.text(270,119,`★ ${totalStars} / ${levels.length*3}    ·    ${unlocked}/${levels.length} desbloqueados`,{fontFamily:"system-ui, sans-serif",fontSize:"13px",color:"#a7b3bf"}).setOrigin(.5);

    const cols=2,cardW=212,cardH=112,gapX=18,gapY=15,startX=270-(cardW+gapX)/2,startY=205;
    visible.forEach((level,localIndex)=>{
      ...
      this.add.text(x-74,y+39,`${GLYPHS[mechanic]}  ${LABELS[mechanic]}`,{fontFamily:"system-ui, sans-serif",fontSize:"12px",fontStyle:"bold",color:isUnlocked?"#a8b7c2":"#45505a"}).setOrigin(0,.5);
      this.add.text(x+82,y+39,`★★★ ${formatRequirement(level.threeStar,true)}`,{fontFamily:"system-ui, sans-serif",fontSize:"12px",color:isUnlocked?"#9baab6":"#414b54"}).setOrigin(1,.5);
