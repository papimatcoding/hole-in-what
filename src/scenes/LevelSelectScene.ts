...
export class LevelSelectScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    visible.forEach((level, localIndex) => {
      ...
      this.add.text(x - 82, y - 32, `G${level.group}`, {
        fontFamily: "system-ui, sans-serif", fontSize: "9px", fontStyle: "bold", color: "#667687"
      }).setOrigin(0, 0.5);

      this.add.text(x, y - 16, String(index + 1), {
        fontFamily: "system-ui, sans-serif", fontSize: "25px", fontStyle: "bold", color: "#f5f7fa"
      }).setOrigin(0.5);

      const stars = "★".repeat(record.stars) + "☆".repeat(3 - record.stars);
      this.add.text(x, y + 22, stars, {
        fontFamily: "system-ui, sans-serif", fontSize: "17px", color: record.stars > 0 ? "#f1d07a" : "#566473"
      }).setOrigin(0.5);
