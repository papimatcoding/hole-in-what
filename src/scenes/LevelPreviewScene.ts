...
export class LevelPreviewScene extends Phaser.Scene{
  ...
  create():void{
    ...
    this.add.text(42,42,"‹",{fontFamily:"system-ui, sans-serif",fontSize:"34px",color:"#f4f7fa"}).setInteractive({useHandCursor:true}).on("pointerup",()=>this.scene.start("editor"));
    ...
    this.add.text(270,920,"CLICK = ABRIR EN EDITOR",{fontFamily:"system-ui, sans-serif",fontSize:"11px",fontStyle:"bold",color:"#718392"}).setOrigin(.5);
  ...
  private card(level:LevelDefinition,index:number):void{
    const col=index%2,row=Math.floor(index/2),x=30+col*250,y=126+row*150,w=230,h=132;
    const bg=this.add.rectangle(x+w/2,y+h/2,w,h,0x131c23).setStrokeStyle(1,0x344553).setInteractive({useHandCursor:true});
    bg.on("pointerover",()=>bg.setFillStyle(0x1b2832));bg.on("pointerout",()=>bg.setFillStyle(0x131c23));bg.on("pointerup",()=>this.scene.start("editor",{loadLevelId:level.id}));
    this.drawMini(level,x+12,y+9,72,114);
  ...
  private drawMini(level:LevelDefinition,x:number,y:number,w:number,h:number):void{
    ...
    for(const r of level.walls??[])rect(r,0x334756,1);for(const r of level.movingWalls??[])rect(r,0x6f8798,1);for(const r of level.popWalls??[])rect(r,0x8f6042,.72);
