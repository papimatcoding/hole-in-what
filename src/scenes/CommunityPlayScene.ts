...
export class CommunityPlayScene extends Phaser.Scene{
  ...
  private async writeComment():Promise<void>{if(!this.mapId)return;const message=window.prompt("Comentario (máx. 500 caracteres)","")?.trim();if(!message)return;const result=await CommunityMaps.comment(this.mapId,message);this.closeOverlay();this.toast(result.ok?"✓ COMENTARIO GUARDADO":"NO SE PUDO GUARDAR",result.ok);this.time.delayedCall(200,()=>{void this.openComments();});}
