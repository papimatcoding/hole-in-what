...
export class CommunityPlayScene extends Phaser.Scene{
  ...
  private openReport():void{
    ...
    const options:[string,CommunityReportCategory][]=[["BUG","bug"],["IMPOSIBLE","impossible"],["INAPROPIADO","inappropriate"],["SPAM","spam"],["OTRO","other"]];options.forEach(([label,category],i)=>{const y=376+i*58;children.push(this.panelButton(270,y,320,label,()=>{if(category==="bug"||category==="impossible"||category==="other")this.openReportNote(category);else void this.sendReport(category,"");},category==="bug"||category==="impossible"));});children.push(this.panelButton(270,690,220,"CERRAR",()=>this.closeOverlay(),false));this.overlayPanel=this.add.container(0,0,children).setDepth(220);
  }
  private openReportNote(category:CommunityReportCategory):void{
    if(!this.mapId)return;this.closeOverlay();const children:Phaser.GameObjects.GameObject[]=[];
    children.push(this.add.rectangle(270,480,540,960,0x05080b,.88).setInteractive(),this.add.rectangle(270,470,440,480,0x111a22,.99).setStrokeStyle(2,0x405668),this.add.text(270,286,"DETALLE DEL REPORTE",{fontFamily:"system-ui",fontSize:"18px",fontStyle:"bold",color:"#f5f7fa"}).setOrigin(.5),this.add.text(270,322,"Opcional · máximo 500 caracteres",{fontFamily:"system-ui",fontSize:"10px",color:"#8397a4"}).setOrigin(.5));
    const area=document.createElement("textarea");area.maxLength=500;area.placeholder="Describe el problema si quieres…";area.spellcheck=true;area.setAttribute("aria-label","Detalle del reporte del mapa");Object.assign(area.style,{width:"350px",height:"150px",boxSizing:"border-box",resize:"none",border:"2px solid #587286",borderRadius:"9px",background:"#0c141a",color:"#eef5f8",font:"500 16px system-ui",padding:"12px 14px",outline:"none",lineHeight:"1.4"});
    children.push(this.add.dom(270,430,area),this.panelButton(170,568,150,"CANCELAR",()=>{this.closeOverlay();this.openReport();},false),this.panelButton(370,568,150,"ENVIAR",()=>{void this.sendReport(category,area.value);},true));this.overlayPanel=this.add.container(0,0,children).setDepth(240);this.time.delayedCall(50,()=>area.focus());
  }
  private async sendReport(category:CommunityReportCategory,note:string):Promise<void>{if(!this.mapId)return;const result=await CommunityMaps.report(this.mapId,category,note.trim().slice(0,500));this.closeOverlay();this.toast(result.ok?"✓ REPORTE ENVIADO":"NO SE PUDO REPORTAR",result.ok);}
