...
export class PatchNotesScene extends Phaser.Scene{
  ...
  create():void{
    setupDesignCamera(this);const desktop=isDesktopUI(),tr=I18n.text;this.cameras.main.setBackgroundColor("#0b0f14");PatchNotes.markRead();
    ...
    for(const note of PATCH_NOTES.slice(0,desktop?3:3)){
      ...
      this.add.text(62,y+24,tr(note.title),{fontFamily:"system-ui",fontSize:uiFontSize(13,2),fontStyle:"bold",color:"#eef4f8"}).setOrigin(0,.5);
      this.add.text(latest?410:478,y+24,tr(note.date),{fontFamily:"system-ui",fontSize:uiFontSize(8,2),fontStyle:"bold",color:"#778a98"}).setOrigin(1,.5);
      this.add.text(62,y+54,tr(note.summary),{fontFamily:"system-ui",fontSize:uiFontSize(10,2),color:"#a9b9c4",wordWrap:{width:408},lineSpacing:2});
      note.bullets.forEach((bullet,i)=>this.add.text(72,y+94+i*(desktop?36:38),`• ${tr(bullet)}`,{fontFamily:"system-ui",fontSize:uiFontSize(9,2),color:"#d1dbe2",wordWrap:{width:390},lineSpacing:1}));
