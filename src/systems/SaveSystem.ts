...
function load():SaveData{
  try{const raw=localStorage.getItem(STORAGE_KEY);if(!raw)return emptySave();const parsed=JSON.parse(raw) as SaveData|SaveDataV3|SaveDataV2|SaveDataV1;if(![1,2,3,4].includes(parsed.version)||typeof parsed.levels!=="object")throw new Error("Invalid save");return migrate(parsed);}catch{return emptySave();}
}
...
function trollUnlockedForSave(save:SaveData):boolean{
  const classic=levelsForMode("classic");
  const stars=classic.reduce((sum,level)=>sum+(save.levels[level.id]?.stars??0),0);
...
export const SaveSystem={
  ...
  submit(levelId:string,stars:number,strokes:number,timeMs:number):SubmitResult{
    const save=load(),current=save.levels[levelId]??emptyRecord(),nextStars=Math.max(current.stars,stars),gainedStars=Math.max(0,nextStars-current.stars);
  ...
  classicProgress():{stars:number;completed:number;total:number;requiredStars:number;firstChapterCompleted:number;requiredCompletions:number}{
    const save=load(),levels=levelsForMode("classic"),stars=levels.reduce((sum,level)=>sum+(save.levels[level.id]?.stars??0),0),completed=levels.reduce((sum,level)=>sum+(save.levels[level.id]?.completed?1:0),0);
    const firstChapterCompleted=levels.slice(0,TROLL_UNLOCK_CLASSIC_COMPLETIONS).reduce((sum,level)=>sum+(save.levels[level.id]?.completed?1:0),0);
  ...
  isLevelUnlocked(mode:GameMode,index:number):boolean{
    const levels=levelsForMode(mode);if(index<0||index>=levels.length)return false;
    const save=load();if(mode==="troll"&&!trollUnlockedForSave(save))return false;if(index===0)return true;
    const current=levels[index]!,previous=levels[index-1]!;
    return save.levels[current.id]?.completed===true||save.levels[previous.id]?.completed===true;
  },
  unlockedLevelCount(mode:GameMode):number{
    const levels=levelsForMode(mode);let count=0;for(let i=0;i<levels.length;i+=1){if(this.isLevelUnlocked(mode,i))count+=1;else break;}return count;
  },
