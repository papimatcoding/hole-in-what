...
export function openBetaReport(scene:Phaser.Scene,context:BetaReportContext,onClose?:()=>void):Phaser.GameObjects.Container{
  ...
  children.push(scene.add.rectangle(270,480,448,462,0x111a22,.99).setStrokeStyle(2,0x496173));
  children.push(scene.add.text(270,284,`REPORTAR · ${context.levelId.toUpperCase()}`,{fontFamily:"system-ui",fontSize:"17px",fontStyle:"bold",color:"#f5f7fa"}).setOrigin(.5));
  children.push(scene.add.text(270,316,"No hace falta completar el hoyo.",{fontFamily:"system-ui",fontSize:"10px",color:"#8da0ad"}).setOrigin(.5));
  CHOICES.forEach(([label,category],i)=>{
    const x=170+(i%2)*200,y=382+Math.floor(i/2)*72;
    const bg=scene.add.rectangle(x,y,180,56,0x1a2731).setStrokeStyle(1,0x52697a).setInteractive({useHandCursor:true});
  ...
  const closeBg=scene.add.rectangle(270,654,190,50,0x141d24).setStrokeStyle(1,0x354652).setInteractive({useHandCursor:true});
  const closeText=scene.add.text(270,654,"CERRAR",{fontFamily:"system-ui",fontSize:"11px",fontStyle:"bold",color:"#94a5b1"}).setOrigin(.5);
  closeBg.on("pointerdown",()=>{closeBg.setScale(.98);closeText.setScale(.98);}).on("pointerout",()=>{closeBg.setScale(1);closeText.setScale(1);}).on("pointerup",()=>{closeBg.setScale(1);closeText.setScale(1);close();});children.push(closeBg,closeText);
