...
function trapTriggerFraction(archetype:TrollTrapArchetype):number {
  switch(archetype){case "floor-drop":case "safe-lane-collapse":return .30;case "gate-pop":case "cross-gate":case "late-combo":return .33;case "bumper-ambush":return .36;case "rebound-punish":return .46;}
}
...
function calibrateHardTrap(level:LevelDefinition,index:number):void {
  ...
  if(archetype==="gate-pop"){level.popWalls=[routeMostlyVertical?{x:anchor.x-66,y:anchor.y-11,w:132,h:22,...triggerBase}:{x:anchor.x-11,y:anchor.y-66,w:22,h:132,...triggerBase}];return;}
  ...
  if(archetype==="cross-gate"){level.popWalls=[routeMostlyVertical?{x:anchor.x-12+normal.x*34*side,y:anchor.y-60,w:24,h:120,...triggerBase}:{x:anchor.x-60,y:anchor.y-12+normal.y*34*side,w:120,h:24,...triggerBase}];return;}
...
export function applyPostAuditBalance(level:LevelDefinition,index:number):LevelDefinition {
  ...
  // Authored vertical-slice holes already contain deliberate geometry, trap timing and pars.
  // They still pass through sanitizeCourse + the same simulation/auditor, but are not mutated
  // by procedural repair rules afterwards.
