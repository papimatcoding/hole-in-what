...
function fanCurve(ctx:Ctx):LevelDefinition{
  ...
  const z=mirrorRect(r(285,500,165,150),ctx.mirror);
...
function portalSlalom(ctx:Ctx):LevelDefinition{
  ...
  level.portals=[portal(mirrorX(390,ctx.mirror),590,mirrorX(145,ctx.mirror),335,27)];
  return level;
}
...
function bridge(ctx:Ctx,trampoline:boolean):LevelDefinition{
  ...
  if(trampoline){
    level.trampolines=[{x:mirrorX(165,ctx.mirror),y:555,r:36,power:445+ctx.difficulty*12}];
  }else{
    const rx=ctx.mirror?318:110;
    level.ramps=[{x:rx,y:530,w:112,h:72,dx:0,dy:-1,lift:350+ctx.difficulty*18,boost:38+ctx.difficulty*8}];
  }
  return level;
}
...
function mastery(ctx:Ctx):LevelDefinition{
  const level=slalom(ctx);
  const z=mirrorRect(r(315,525,135,125),ctx.mirror);
...
function addTrap(level:LevelDefinition,ctx:Ctx):void{
  ...
  if(kind===0){
    level.popWalls=[{x:anchorX-68,y:anchorY,w:136,h:22,triggerX:anchorX,triggerY,triggerRadius:92}];
  }else if(kind===1){
    level.popBumpers=[{x:anchorX,y:anchorY,r:34,triggerX:anchorX,triggerY,triggerRadius:96}];
  }else{
    level.popVoids=[{x:anchorX-52,y:anchorY-26,w:104,h:52,triggerX:anchorX,triggerY,triggerRadius:98}];
  }
}
...
export function buildHardCourse(index:number):LevelDefinition{
  ...
  if(index>=6 && family!==1 && family!==5){
    const z=mirrorRect(r(315,600,130,110),ctx.mirror);
    level.fans=[fan(z.x,z.y,z.w,z.h,ctx.mirror?-0.72:0.72,-0.25,300+ctx.difficulty*25)];
  }
  if(index>=11 && family!==2 && index%4===0){
    level.portals=[portal(mirrorX(395,ctx.mirror),620,mirrorX(130,ctx.mirror),300,27)];
  }
