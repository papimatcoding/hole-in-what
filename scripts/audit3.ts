...
interface NamedRect { label:string; kind:string; r:RectDef; structural:boolean; visual:boolean; }
interface NamedCircle { label:string; kind:string; x:number; y:number; r:number; }
...
function rects(level:LevelDefinition):NamedRect[] {
  const out:NamedRect[]=[];
  const add=(kind:string,arr:RectDef[]|undefined,structural:boolean,visual=true)=>arr?.forEach((r,i)=>out.push({label:`${kind}[${i}]`,kind,r,structural,visual}));
...
const rangeOverlap=(a1:number,a2:number,b1:number,b2:number)=>Math.max(0,Math.min(a2,b2)-Math.max(a1,b1));
...
function segmentRectApproxDistance(a:Vec2,b:Vec2,r:RectDef):number {
  let best=Infinity; for(let i=0;i<=10;i++){const q=i/10,p={x:a.x+(b.x-a.x)*q,y:a.y+(b.y-a.y)*q};best=Math.min(best,rectDistanceToPoint(r,p));} return best;
}
...
function structuralAt(level:LevelDefinition,p:Vec2):boolean {
  ...
  if(circles(level).filter(c=>c.kind==="bumper"||c.kind==="movingBumper"||c.kind==="popBumper").some(c=>Math.hypot(p.x-c.x,p.y-c.y)<=c.r))return true;
  return false;
}
...
function deadGaps(level:LevelDefinition):GapIssue[] {
  const rs=structuralRects(level),cs=circles(level).filter(c=>c.kind==="bumper"||c.kind==="movingBumper"||c.kind==="popBumper"),issues:GapIssue[]=[];
  ...
  for(const a of rs){
    if(a.r.h>=BALL*2){add(`${a.label} ↔ left boundary`,a.r.x-FIELD.x,"edge slit");add(`${a.label} ↔ right boundary`,FIELD.x+FIELD.w-(a.r.x+a.r.w),"edge slit");}
    if(a.r.w>=BALL*2){add(`${a.label} ↔ top boundary`,a.r.y-FIELD.y,"edge slit");add(`${a.label} ↔ bottom boundary`,FIELD.y+FIELD.h-(a.r.y+a.r.h),"edge slit");}
  }
  for(const c of cs)for(const a of rs)add(`${c.label} ↔ ${a.label}`,rectDistanceToPoint(a.r,{x:c.x,y:c.y})-c.r,"circle gap");
  return issues.sort((a,b)=>a.clearance-b.clearance);
}
function routePoints(level:LevelDefinition):Vec2[] {
  const src=level.designPath?.length?level.designPath:[level.ball,level.hole],out:Vec2[]=[];
  for(const p of src)if(!out.length||dist(out.at(-1)!,p)>2)out.push(p);
...
function routeMetrics(level:LevelDefinition):RouteMetrics {
  const pts=routePoints(level),vectors=pts.slice(1).map((p,i)=>({x:p.x-pts[i]!.x,y:p.y-pts[i]!.y})); let length=0,axis=0,diag=0;
  for(const v of vectors){const l=Math.hypot(v.x,v.y);length+=l;const a=((Math.atan2(v.y,v.x)*180/Math.PI)%90+90)%90,dAxis=Math.min(a,90-a);if(dAxis<=12)axis++;if(dAxis>=24)diag++;}
  let turns=0,hardTurns=0; for(let i=1;i<vectors.length;i++){const a=vectors[i-1]!,b=vectors[i]!,den=Math.hypot(a.x,a.y)*Math.hypot(b.x,b.y)||1,turn=Math.acos(clamp((a.x*b.x+a.y*b.y)/den,-1,1))*180/Math.PI;if(turn>=25)turns++;if(turn>=65)hardTurns++;}
...
function offRouteObjects(level:LevelDefinition):string[] {
  const pts=routePoints(level),out:string[]=[];for(const obj of structuralRects(level)){let best=Infinity;for(let i=1;i<pts.length;i++)best=Math.min(best,segmentRectApproxDistance(pts[i-1]!,pts[i]!,obj.r));const center={x:obj.r.x+obj.r.w/2,y:obj.r.y+obj.r.h/2};if(best>135&&dist(center,level.ball)>95&&dist(center,level.hole)>95)out.push(obj.label);}return out;
}
function silhouette(level:LevelDefinition,hRatio:number,vRatio:number,open:number):string {
  const walls=structuralRects(level).filter(x=>x.kind.includes("wall")||x.kind.includes("Wall")),tris=level.triangles?.length??0,curves=level.curves?.length??0,circs=circles(level).filter(x=>x.kind!=="portal").length;
...
function geometryMetrics(level:LevelDefinition):GeometryMetrics {
  ...
  const occ=occupancy(level),spread=objectSpread(level),open=1-occ.coverage,structuralCount=walls.length+tris+curves+circs,hRatio=walls.length?h/walls.length:0,vRatio=walls.length?v/walls.length:0;
...
function mechanics(level:LevelDefinition):Set<string> {
  const out=new Set<string>(),add=(name:string,n:number|undefined)=>{if((n??0)>0)out.add(name);};
  add("bumper",(level.bumpers?.length??0)+(level.movingBumpers?.length??0)+(level.popBumpers?.length??0));add("sand",level.sand?.length);add("ice",level.ice?.length);add("booster",level.boosters?.length);add("fan",(level.fans?.length??0)+(level.winds?.length??0));add("curve",level.curves?.length);add("portal",level.portals?.length);add("moving",(level.movingWalls?.length??0)+(level.movingBumpers?.length??0));add("void",(level.voids?.length??0)+(level.popVoids?.length??0));add("ramp",level.ramps?.length);add("trampoline",level.trampolines?.length);return out;
}
...
function compareLevels(a:LevelDefinition,ag:GeometryMetrics,ar:RouteMetrics,b:LevelDefinition,bg:GeometryMetrics,br:RouteMetrics):SimilarityHit {
  ...
  const score=clamp(shape*.46+raster*.28+mechanic*.14+(silhouetteSame?.12:0),0,1);
  return{id:b.id,score:round(score,3),shape:round(shape,3),raster:round(raster,3),mechanic:round(mechanic,3),silhouetteSame,distance:Math.abs(levelIndex(a)-levelIndex(b))};
}
function profile(row:HumanRow|null,name:string):HumanProfile|null { return row?.profiles.find(x=>x.name===name)??null; }
function scoreCard(level:LevelDefinition,g:GeometryMetrics,r:RouteMetrics,h:HumanRow|null,recent:SimilarityHit|null):ScoreCard {
  const touch=profile(h,"touch")?.successRate??.7,casual=profile(h,"casual")?.successRate??.6,tol=h?.minShotTolerance??.5,recovery=h?.recovery.recoverableRate??.8,sealed=g.deadGaps.filter(x=>x.kind==="SEALED").length,tight=g.deadGaps.length-sealed;
...
function designerAdvice(level:LevelDefinition,g:GeometryMetrics,r:RouteMetrics,h:HumanRow|null,recent:SimilarityHit|null,s:ScoreCard,fb:FeedbackLevel|null):Advice[] {
  const out:Advice[]=[],push=(priority:Priority,code:string,text:string,evidence:string)=>out.push({priority,code,text,evidence});
  if(h?.status==="BLOCKER")push("P0","HUMAN_BLOCKER","Resolver el blocker de ejecución antes de tocar estética o dificultad.",h.flags.join(", ")||"Audit 2.1 BLOCKER");
  ...
  if(tight.length>=2)push("P2","TIGHT_GAPS","Revisar pasos casi transitables; la geometría debe comunicar qué es corredor y qué no.",tight.slice(0,4).map(x=>`${x.between} ${x.clearance}px`).join("; "));
  if(g.rectDominance>=.86&&g.shapeCount>=3)push("P1","RECT_DOMINANCE","Romper la gramática de cajas con una pieza diagonal, triangular, curva o circular cuando cambie realmente la lectura/rebote.",`${Math.round(g.rectDominance*100)}% de primitivas visuales rectangulares`);
  if(g.silhouette==="SHELVES"&&r.sideChanges>=2)push("P1","SHELF_GRAMMAR","Evitar otra secuencia de estanterías alternas. Mantener la mecánica, cambiar la silueta/pregunta espacial.",`${Math.round(g.horizontalWallRatio*100)}% paredes horizontales · ${r.sideChanges} cambios de lado`);
  if(r.axisRatio>=.8&&r.segments>=3)push("P2","AXIS_ROUTE","Valorar una decisión diagonal o curva; demasiados tramos ortogonales refuerzan sensación de plantilla.",`${Math.round(r.axisRatio*100)}% de segmentos alineados a ejes`);
  if(recent&&recent.score>=.76)push("P1","PERCEPTUAL_REPEAT","Diferenciar este hoyo de sus vecinos por silueta, escala o ruta; mover coordenadas no basta.",`${recent.id} similarity=${recent.score}`);
  if(g.offRouteObjects.length)push("P2","OFF_ROUTE_OBJECT","Revisar si estas piezas son anti-cheese, lectura o recuperación. Si no cumplen una función, eliminarlas.",g.offRouteObjects.slice(0,5).join(", "));
  ...
  if((h?.routeFamilies??2)<=1&&!level.onboarding)push("P1","SINGLE_ROUTE","Crear otra familia de ejecución o una elección riesgo/recompensa para evitar niveles-contraseña.",`${h?.routeFamilies??0} familia(s)`);
  const touch=profile(h,"touch")?.successRate??1;if(touch<.45)push("P1","LOW_TOUCH_MARGIN","Aumentar zona de aterrizaje o anchura útil; subir diversión con decisiones, no con precisión obligatoria.",`touch=${pct(touch)} tolerance=${pct(h?.minShotTolerance??null)}`);
  if((h?.recovery.recoverableRate??1)<.78)push("P1","POOR_RECOVERY","Dar salida a tiros mediocres: que el error cueste golpes, no una run muerta.",`recovery=${pct(h?.recovery.recoverableRate??null)}`);
  if(level.threeStar.maxStrokes!==undefined&&h?.humanStrokes!==null&&h?.humanStrokes!==undefined&&Math.abs(level.threeStar.maxStrokes-h.humanStrokes)>=2)push("P1","OBJECTIVE_MISMATCH","Calibrar 3★ con la ruta humana; trickshots expertos pueden existir sin definir el par.",`target=${level.threeStar.maxStrokes} human=${h.humanStrokes}`);
  ...
  if(fb&&(fb.avgOriginality??5)<=2.5)push("P1","HUMAN_ORIGINALITY_LOW","Cambiar silueta o pregunta estratégica: jugadores reales perciben repetición.",`originality=${fb.avgOriginality?.toFixed(1)} n=${fb.sampleSize}`);
  ...
  if(!out.length)push("P2","KEEP_CORE","No hay deuda clara. Mantener la pregunta central y validar con humanos antes de añadir piezas.",`overall=${s.overall}`);
  const order:Record<Priority,number>={P0:0,P1:1,P2:2};return out.sort((a,b)=>order[a.priority]-order[b.priority]);
}

const metrics=new Map<string,{g:GeometryMetrics;r:RouteMetrics}>();for(const level of levels)metrics.set(level.id,{g:geometryMetrics(level),r:routeMetrics(level)});
const rows:Audit3Row[]=[];
for(const level of levels){
  const {g,r}=metrics.get(level.id)!,h=humanById.get(level.id)??null,fb=feedbackById.get(level.id)??null,peers=levels.filter(x=>x.mode===level.mode&&x.id!==level.id);
  const hits=peers.map(p=>{const m=metrics.get(p.id)!;return compareLevels(level,g,r,p,m.g,m.r);}).sort((a,b)=>b.score-a.score),nearest=hits[0]??null,recent=hits.filter(x=>x.distance<=3).sort((a,b)=>b.score-a.score)[0]??null,s=scoreCard(level,g,r,h,recent),adv=designerAdvice(level,g,r,h,recent,s,fb);
...
function campaignSummary(mode:string):CampaignSummary {
  ...
  const repeatedSilhouetteRuns=consecutiveRuns(rs,x=>x.geometry.silhouette),repeatedScaleRuns=consecutiveRuns(rs,x=>x.geometry.scaleClass,4),rectDominantLevels=rs.filter(x=>x.geometry.rectDominance>=.86&&x.geometry.shapeCount>=3).length,nonRectLevels=rs.filter(x=>x.geometry.nonRectRatio>=.18).length,recommendations:string[]=[];
  if(rectDominantLevels/Math.max(1,rs.length)>.5)recommendations.push(`Demasiados hoyos rectangulares (${rectDominantLevels}/${rs.length}). En próximos batches, al menos 2 de cada 3 deberían usar una silueta no basada solo en rectángulos.`);
  ...
  if(repeatedScaleRuns.length)recommendations.push(`Variar escala aparente: ${repeatedScaleRuns.map(x=>`${x.from}–${x.to} ${x.value}`).join(", ")}. Ninguna escala debería dominar 4+ hoyos seguidos.`);
  if(rs.some(x=>x.geometry.deadGaps.some(g=>g.kind==="SEALED")))recommendations.push("Aplicar regla de clearance visual: hueco aparentemente jugable ≥34px; si no, cerrarlo de forma inequívoca.");
  recommendations.push("Cada hoyo debe poder describirse por una frase espacial antes de hablar de mecánica: isla, V, embudo, anillo, campo abierto, diagonal, etc.");
  recommendations.push("Las estrellas siguen la ruta humana; un trickshot del solver puede vivir como mastery line sin deformar el mapa para eliminarlo.");
