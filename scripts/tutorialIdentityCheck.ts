...
assert.equal(SaveSystem.isCampaignLevelUnlocked(16),false,"chapter star requirement only makes the chapter claimable");
assert.deepEqual(SaveSystem.prestigeRewardState("chapter-metropolis"),{eligible:true,claimed:false},"Metropolis becomes ready to claim");
assert.equal(SaveSystem.claimPrestigeReward("chapter-metropolis").ok,true,"claiming Metropolis unlocks the chapter");
assert.equal(SaveSystem.isCampaignLevelUnlocked(16),true,"claimed chapter opens former H01");
...
SaveSystem.submit("classic-02",3,2,1000);
SaveSystem.submit("classic-03",3,2,1000);
assert.equal(SaveSystem.totalStarsAll(),9,"reward setup stays one star below the first milestone");
assert.equal(SaveSystem.isOwned("trail-stardust"),false,"reward stays locked below 10 stars");
const milestone=SaveSystem.submit("classic-04",3,2,1000);
assert.deepEqual(milestone.newlyUnlockedCosmetics,[],"crossing 10 stars does not auto-grant Prestige rewards");
assert.deepEqual(SaveSystem.prestigeRewardState("prestige-trail-stardust"),{eligible:true,claimed:false},"10 stars makes Stardust claimable");
assert.equal(SaveSystem.isOwned("trail-stardust"),false,"claimable reward remains unowned until claimed");
assert.equal(SaveSystem.claimPrestigeReward("prestige-trail-stardust").ok,true,"player explicitly claims Stardust");
assert.equal(SaveSystem.equip("trail","trail-stardust").trail,"trail-stardust","claimed reward can be equipped");
const starsBeforeReplay=SaveSystem.totalStarsAll(),walletBeforeReplay=SaveSystem.wallet();
const replayReward=SaveSystem.submit("classic-04",2,3,1500);
...
assert.equal(SaveSystem.claimPrestigeReward("prestige-trail-stardust").ok,false,"claimed Prestige reward cannot be claimed twice");
assert.deepEqual(SaveSystem.claimEligibleStarRewards(),[],"legacy auto-claim hook is intentionally inert");
